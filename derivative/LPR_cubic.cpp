...
:Xp(Xn),h(hn),n(nn)
{
    ...
    *argmin = 1;
    ...
    for (int i = 1; i < 20; i++) {      // Here we try a range of bandwiths and compute the residual sum of square of the regression for each
        *d = obj.RSS(0.1 * i);
        if (*d < *min) {
            *min = *d;
            *argmin = 0.1 * i;
...
double LPR_cubic::value(double x)
{
    ...
    for(int i = 0 ; i < n ; i++){
        ...
        *(weights+i) = 1.0/sqrt(2*3.14159)*exp(-1*(*dif_squared)/pow(h,2)); //Use of Gaussian kernel
    }
